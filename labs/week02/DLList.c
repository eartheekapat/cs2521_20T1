// DlistList.c - Implementation of doubly-linked list ADT
...
void DLListAfter (DLList L, char *it)
{
    ...
    if (L->curr == NULL) {
        ...
        //Insert at the last node of the list
            new->next = NULL;
			new->prev = L->curr;
            L->curr->next = new;
            L->last = L->curr = new;
...
void DLListDelete (DLList L)
{
	...
	if (DLListIsEmpty(L)) {
		...
		DLListNode *temp = L->curr; // temp pointer
		...
		if (L->first->next == NULL) {
			...
			L->nitems--;
			return;
		} else if (L->curr == L->last) { // the node is the last node of the list
			L->last = L->curr->prev;
			L->curr = L->curr->prev;
			L->curr->next = NULL;
			freeDLListNode(temp);
			L->nitems--;
			return;
		} else if (L->curr == L->first) { // the node is the first node of the list
			L->first = L->curr->next;
			L->curr = L->curr->next;
			L->curr->prev = NULL;
			freeDLListNode(temp);
			L->nitems--;
			return;
		} else { // the node is somwhere in the list
