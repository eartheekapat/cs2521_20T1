...
{
	if (IntListIsSorted(L) != true) return;
    //check if the list is empty
	if (L->first == NULL) {
		IntListInsert(L, v);
	//v is largest
	} else if (L->last->data <= v) { // check if v is >= to the last node's data
		IntListInsert (L, v);
	// v is smallest
	} else if (v < L->first->data) { 
		struct IntListNode *new = newIntListNode (v); // create new node
		// check if the new node is smallest
		new->next = L->first;
		L->first = new;
		L->size++;
	} else {
		struct IntListNode *curr = L->first;
		while (curr != NULL) { // traversing thorugh the list
